...
Current version in this guide: `v0.9.0`
...
```bash
cd /opt
git clone --branch v0.9.0 --depth 1 https://github.com/Kobii-git/Rackpad.git
cd Rackpad
```
...
When a newer release exists, replace `v0.9.0` with the newer version tag.
...
```bash
cd /opt
git clone --branch v0.9.0 --depth 1 https://github.com/Kobii-git/Rackpad.git rackpad
cd /opt/rackpad
```
...
11. Open `WiFi` and confirm APs or wireless clients appear in their own workspace when placed there.
12. Run a discovery scan against a small subnet, confirm duplicate warnings work, and import one discovered host.
13. Open `Users` as an admin and download a JSON backup.
